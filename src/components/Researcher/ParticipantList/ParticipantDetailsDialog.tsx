...
export default function ParticipantDetailsDialog({
  ...
  const handleSave = async () => {
    try {
      ...
      await LAMP.Participant.update(participant.id, formState)
      await LAMP.Participant.update(participant.id, formState)
        .then((res) => {
          console.log("update sends", formState, "receives", res, "id", participant.id)
        })
        .catch((error) => {
          console.log("update send", formState, "error", error)
        })

      LAMP.Participant.view(participant.id).then((res) => {
      ...
      //   Service.updateValue("participants", { participants: [{ ...participant, ...formState, id: participant.id }] }, "is_deleted", "id")
      // LAMP.Participant.update(participant.id, formState).then((res)=>{
      //     Service.updateValue("participants", { participants: [{ username : formState.firstName + formState.lastName, id: participant.id }] }, "username", "id")
      //     console.log("updateValue : participants | ", participant.id, res)
      // }).catch(()=>{
      //     console.log("updateValue : participants | ", formState.firstName + formState.lastName, participant.id)
      // })
      ...
      enqueueSnackbar(`Failed to update participant: ${error.message}`, { variant: "error" })
    }
  }
        ...
        <Grid container spacing={3}>
            ...
            <Grid container spacing={2}>
              {/* <Grid item xs={12}>
                <Box display="flex" alignItems="center">
                  <Typography className={classes.switchLabel}>Study ID</Typography>
                  <Typography variant="subtitle2">Created On</Typography>
                  <Typography>
                    {formatDate_detailed(formState.systemTimestamps?.createdAt)}
                  </Typography>
                </Box>
              </Grid> */}
              <Grid item xs={6}>
          ...
          <Grid item xs={12} className={classes.section}>
            <TextField
              ...
              multiline
              rows={3}
              label="Researcher's Notes"
              value={formState.researcherNote || ""}
              disabled={!isEditing}
              onChange={(e) => handleFormChange("researcherNote", e.target.value)}
            />
          </Grid>
          ...
          <Grid item xs={12} className={classes.section}>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={6}>
                <Typography variant="subtitle2">Created On</Typography>
                <Typography>{formatDate_detailed(formState.systemTimestamps?.createdAt)}</Typography>
              </Grid>
              <Grid item xs={6}>
                <Typography variant="subtitle2">Last Login</Typography>
                <Typography>{formatDate_detailed(formState.systemTimestamps?.lastLoginTime)}</Typography>
              </Grid>
              <Grid item xs={6}>
                <Typography variant="subtitle2">Last Activity</Typography>
                <Typography>{formatDate_detailed(formState.systemTimestamps?.lastActivityTime)}</Typography>
              </Grid>
              {formState.systemTimestamps?.suspensionTime && (
                <Grid item xs={6}>
                  <Typography variant="subtitle2">Suspended On</Typography>
                  <Typography>{formatDate_detailed(formState.systemTimestamps.suspensionTime)}</Typography>
                </Grid>
              )}
            </Grid>
          </Grid>
